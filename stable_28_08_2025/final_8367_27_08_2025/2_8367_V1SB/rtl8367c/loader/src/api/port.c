...
code uint8 Fiber[FIBER_INIT_SIZE] = {
...
static rtk_api_ret_t _rtk_port_FiberModeAbility_get(rtk_port_t port, rtk_port_phy_ability_t *pAbility)
{
    ...
    /* Speed ability */
    if ((retVal = rtl8367c_getAsicRegBit(RTL8367C_REG_FIBER_CFG_1, RTL8367C_SDS_FRC_MODE_OFFSET, &data1)) != RT_ERR_OK)
            return retVal;
    ...
    {
        if ((retVal = rtl8367c_getAsicRegBits(RTL8367C_REG_FIBER_CFG_1, RTL8367C_SDS_MODE_MASK, &data1)) != RT_ERR_OK)
            return retVal;
        ...
        if(data1 == 4)
        {
            ...
            if ((retVal = rtl8367c_getAsicReg(RTL8367C_REG_FIB0_CFG00, &data1)) != RT_ERR_OK)
                return retVal;
...
rtk_api_ret_t rtk_port_macForceLinkExt_get(rtk_port_t port, rtk_mode_ext_t *pMode, rtk_port_mac_ability_t *pPortability)
{
    ...
    if ((retVal = rtl8367c_getAsicPortExtMode(ext_id, (uint32 *)pMode)) != RT_ERR_OK)
        return retVal;
...
rtk_api_ret_t rtk_port_macLocalLoopbackEnable_set(rtk_port_t port, rtk_enable_t enable)
{
    ...
    {
        if ((retVal = rtl8367c_getAsicPHYReg(rtk_switch_port_L2P_get(port), PHY_CONTROL_REG, &data1)) != RT_ERR_OK)
            return retVal;
        ...
        if ((retVal = rtl8367c_setAsicPHYReg(rtk_switch_port_L2P_get(port), PHY_CONTROL_REG, data1)) != RT_ERR_OK)
            return retVal;
    }
...
rtk_api_ret_t rtk_port_adminEnable_set(rtk_port_t port, rtk_enable_t enable)
{
    ...
    if ((retVal = rtk_port_phyReg_get(port, PHY_CONTROL_REG, &data1)) != RT_ERR_OK)
        return retVal;
    ...
    if ((retVal = rtk_port_phyReg_set(port, PHY_CONTROL_REG, data1)) != RT_ERR_OK)
        return retVal;
...
rtk_api_ret_t rtk_port_phyComboPortMedia_set(rtk_port_t port, rtk_port_media_t media)
{
    ...
    if(media == PORT_MEDIA_FIBER)
    {
        ...
        {
            if ((retVal = rtl8367c_setAsicReg(0xE000 + idx, (uint32)Fiber[idx])) != RT_ERR_OK)
                return retVal;
        }
...
rtk_api_ret_t rtk_port_phyComboPortMedia_get(rtk_port_t port, rtk_port_media_t *pMedia)
{
    ...
    {
    if ((retVal = rtl8367c_getAsicRegBit(RTL8367C_REG_UTP_FIB_DET, RTL8367C_UTP_FIRST_OFFSET, &data1))!=RT_ERR_OK)
        return retVal;
 ...
 *      a RT_ERR_OK.
 *      The result is stored at pRtctResult->rtk_ge_result
...
rtk_api_ret_t rtk_port_rtctResult_get(rtk_port_t port, rtk_rtctResult_t *pRtctResult)
{
    ...
    pRtctResult->result.rtk_ge_result.channelALen = result.channelALen;
    pRtctResult->result.rtk_ge_result.channelBLen = result.channelBLen;
    pRtctResult->result.rtk_ge_result.channelCLen = result.channelCLen;
    pRtctResult->result.rtk_ge_result.channelDLen = result.channelDLen;

    pRtctResult->result.rtk_ge_result.channelALinedriver = result.channelALinedriver;
    pRtctResult->result.rtk_ge_result.channelBLinedriver = result.channelBLinedriver;
    pRtctResult->result.rtk_ge_result.channelCLinedriver = result.channelCLinedriver;
    pRtctResult->result.rtk_ge_result.channelDLinedriver = result.channelDLinedriver;

    pRtctResult->result.rtk_ge_result.channelAMismatch = result.channelAMismatch;
    pRtctResult->result.rtk_ge_result.channelBMismatch = result.channelBMismatch;
    pRtctResult->result.rtk_ge_result.channelCMismatch = result.channelCMismatch;
    pRtctResult->result.rtk_ge_result.channelDMismatch = result.channelDMismatch;

    pRtctResult->result.rtk_ge_result.channelAOpen = result.channelAOpen;
    pRtctResult->result.rtk_ge_result.channelBOpen = result.channelBOpen;
    pRtctResult->result.rtk_ge_result.channelCOpen = result.channelCOpen;
    pRtctResult->result.rtk_ge_result.channelDOpen = result.channelDOpen;

    pRtctResult->result.rtk_ge_result.channelAShort = result.channelAShort;
    pRtctResult->result.rtk_ge_result.channelBShort = result.channelBShort;
    pRtctResult->result.rtk_ge_result.channelCShort = result.channelCShort;
    pRtctResult->result.rtk_ge_result.channelDShort = result.channelDShort;
