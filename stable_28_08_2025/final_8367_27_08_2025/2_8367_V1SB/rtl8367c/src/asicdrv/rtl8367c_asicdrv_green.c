 ...
 */
ret_t rtl8367c_getAsicGreenPortPage(uint32 port, uint32* pPage)
{
    ret_t retVal;
    uint32 regData;
    uint32 pageMeter;
 ...
 *      None
 */
ret_t rtl8367c_setAsicGreenTrafficType(uint32 priority, uint32 traffictype)
{
 ...
 *      None
 */
ret_t rtl8367c_getAsicGreenTrafficType(uint32 priority, uint32* pTraffictype)
{
    if(priority > RTL8367C_PRIMAX)
        return RT_ERR_QOS_INT_PRIORITY;
 ...
 *      None
 */
ret_t rtl8367c_setAsicGreenHighPriorityTraffic(uint32 port)
{
    if(port > RTL8367C_PORTIDMAX)
        return RT_ERR_PORT_ID;
 ...
 *      None
 */
ret_t rtl8367c_getAsicGreenHighPriorityTraffic(uint32 port, uint32* pIndicator)
{
    if(port > RTL8367C_PORTIDMAX)
        return RT_ERR_PORT_ID;
...
/*
@func int32 | rtl8367c_setAsicGreenEthernet | Set green ethernet function.
...
*/
ret_t rtl8367c_setAsicGreenEthernet(uint32 port,uint32 green)
{
    ret_t retVal;
    uint32 checkCounter;
    uint32 regData;
    uint32 phy_status;
    uint32 patchData[6][2] = { {0x809A, 0x8911}, {0x80A3, 0x9233}, {0x80AC, 0xA444}, {0x809F, 0x6B20}, {0x80A8, 0x6B22}, {0x80B1, 0x6B23} };
    uint32 idx;
    uint32 data1;
...
/*
@func int32 | rtl8367c_getAsicGreenEthernet | Get green ethernet function.
@parm uint32 | *green | Green feature function usage 1:enable 0:disable.
@rvalue RT_ERR_OK | Success.
@rvalue RT_ERR_SMI | SMI access error.
@comm
 ...
 ports will enter power savining mode in 10 seconds after the cable disconnected if power saving function is enabled.
*/
ret_t rtl8367c_getAsicGreenEthernet(uint32 port,uint32* green)
{
    ret_t retVal;
    uint32 regData;
...
*/
ret_t rtl8367c_setAsicPowerSaving(uint32 phy, uint32 enable)
{
    ...
    phyData = phyData & ~(0x0001 << 2UL);
    phyData = phyData | (enable << 2UL);
...
*/
ret_t rtl8367c_getAsicPowerSaving(uint32 phy, uint32* enable)
{
    rtk_api_ret_t retVal;
    uint32 phyData;
