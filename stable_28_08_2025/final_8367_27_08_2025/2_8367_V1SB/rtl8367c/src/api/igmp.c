...
rtk_api_ret_t rtk_igmp_fastLeave_set(rtk_enable_t state)
{
    ...
    if ((retVal = rtl8367c_setAsicIGMPFastLeaveEn((uint32)state))!=RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_igmp_maxGroup_set(rtk_port_t port, uint32 group)
{
    rtk_api_ret_t   retVal;
 ...
 */
rtk_api_ret_t rtk_igmp_maxGroup_get(rtk_port_t port, uint32 *pGroup)
{
    rtk_api_ret_t   retVal;
 ...
 */
rtk_api_ret_t rtk_igmp_currentGroup_get(rtk_port_t port, uint32 *pGroup)
{
    rtk_api_ret_t   retVal;
...
rtk_api_ret_t rtk_igmp_tableFullAction_set(rtk_igmp_tableFullAction_t action)
{
    ...
    if ((retVal = rtl8367c_setAsicIGMPTableFullOP((uint32)action))!=RT_ERR_OK)
        return retVal;
...
rtk_api_ret_t rtk_igmp_tableFullAction_get(rtk_igmp_tableFullAction_t *pAction)
{
    ...
    if ((retVal = rtl8367c_getAsicIGMPTableFullOP((uint32 *)pAction))!=RT_ERR_OK)
        return retVal;
...
rtk_api_ret_t rtk_igmp_checksumErrorAction_set(rtk_igmp_checksumErrorAction_t action)
{
    ...
    if ((retVal = rtl8367c_setAsicIGMPCRCErrOP((uint32)action))!=RT_ERR_OK)
        return retVal;
...
rtk_api_ret_t rtk_igmp_checksumErrorAction_get(rtk_igmp_checksumErrorAction_t *pAction)
{
    ...
    if ((retVal = rtl8367c_getAsicIGMPCRCErrOP((uint32 *)pAction))!=RT_ERR_OK)
        return retVal;
 ...
 * Note:
 */
rtk_api_ret_t rtk_igmp_leaveTimer_set(uint32 timer)
{
    rtk_api_ret_t   retVal;
 ...
 * Note:
 */
rtk_api_ret_t rtk_igmp_leaveTimer_get(uint32 *pTimer)
{
    rtk_api_ret_t   retVal;
 ...
 * Note:
 */
rtk_api_ret_t rtk_igmp_queryInterval_set(uint32 interval)
{
    rtk_api_ret_t   retVal;
 ...
 * Note:
 */
rtk_api_ret_t rtk_igmp_queryInterval_get(uint32 *pInterval)
{
    rtk_api_ret_t   retVal;
 ...
 * Note:
 */
rtk_api_ret_t rtk_igmp_robustness_set(uint32 robustness)
{
    rtk_api_ret_t   retVal;
 ...
 * Note:
 */
rtk_api_ret_t rtk_igmp_robustness_get(uint32 *pRobustness)
{
    rtk_api_ret_t   retVal;
...
rtk_api_ret_t rtk_igmp_suppressionEnable_set(rtk_enable_t reportSuppression, rtk_enable_t leaveSuppression)
{
    ...
    if ((retVal = rtl8367c_setAsicIGMPSuppression((uint32)reportSuppression, (uint32)leaveSuppression))!=RT_ERR_OK)
        return retVal;
...
rtk_api_ret_t rtk_igmp_suppressionEnable_get(rtk_enable_t *pReportSuppression, rtk_enable_t *pLeaveSuppression)
{
    ...
    if ((retVal = rtl8367c_getAsicIGMPSuppression((uint32 *)pReportSuppression, (uint32 *)pLeaveSuppression))!=RT_ERR_OK)
        return retVal;
...
rtk_api_ret_t rtk_igmp_portRxPktEnable_set(rtk_port_t port, rtk_igmp_rxPktEnable_t *pRxCfg)
{
    ...
    if ((retVal = rtl8367c_setAsicIGMPQueryRX(rtk_switch_port_L2P_get(port), (uint32)pRxCfg->rxQuery))!=RT_ERR_OK)
        return retVal;

    if ((retVal = rtl8367c_setAsicIGMPReportRX(rtk_switch_port_L2P_get(port), (uint32)pRxCfg->rxReport))!=RT_ERR_OK)
        return retVal;

    if ((retVal = rtl8367c_setAsicIGMPLeaveRX(rtk_switch_port_L2P_get(port), (uint32)pRxCfg->rxLeave))!=RT_ERR_OK)
        return retVal;

    if ((retVal = rtl8367c_setAsicIGMPMRPRX(rtk_switch_port_L2P_get(port), (uint32)pRxCfg->rxMRP))!=RT_ERR_OK)
        return retVal;

    if ((retVal = rtl8367c_setAsicIGMPMcDataRX(rtk_switch_port_L2P_get(port), (uint32)pRxCfg->rxMcast))!=RT_ERR_OK)
        return retVal;
...
rtk_api_ret_t rtk_igmp_portRxPktEnable_get(rtk_port_t port, rtk_igmp_rxPktEnable_t *pRxCfg)
{
    ...
    if ((retVal = rtl8367c_getAsicIGMPQueryRX(rtk_switch_port_L2P_get(port), (uint32 *)&(pRxCfg->rxQuery)))!=RT_ERR_OK)
        return retVal;

    if ((retVal = rtl8367c_getAsicIGMPReportRX(rtk_switch_port_L2P_get(port), (uint32 *)&(pRxCfg->rxReport)))!=RT_ERR_OK)
        return retVal;

    if ((retVal = rtl8367c_getAsicIGMPLeaveRX(rtk_switch_port_L2P_get(port), (uint32 *)&(pRxCfg->rxLeave)))!=RT_ERR_OK)
        return retVal;

    if ((retVal = rtl8367c_getAsicIGMPMRPRX(rtk_switch_port_L2P_get(port), (uint32 *)&(pRxCfg->rxMRP)))!=RT_ERR_OK)
        return retVal;

    if ((retVal = rtl8367c_getAsicIGMPMcDataRX(rtk_switch_port_L2P_get(port), (uint32 *)&(pRxCfg->rxMcast)))!=RT_ERR_OK)
        return retVal;
 ...
 *
 */
rtk_api_ret_t rtk_igmp_groupInfo_get(uint32 index, rtk_igmp_groupInfo_t *pGroup)
{
    rtk_api_ret_t   retVal;
    uint32      valid;
...
rtk_api_ret_t rtk_igmp_dropLeaveZeroEnable_get(rtk_enable_t *pEnabled)
{
    ...
    if ((retVal = rtl8367c_getAsicIGMPDropLeaveZero((uint32 *)pEnabled))!=RT_ERR_OK)
        return retVal;
