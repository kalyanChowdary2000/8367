...
rtk_api_ret_t rtk_trunk_port_set(rtk_trunk_group_t trk_gid, rtk_portmask_t *pTrunk_member_portmask)
{
    ...
    if((type == 0) || (type == 1))
    {
        if ((pmsk | RTL8367C_PORT_TRUNK_GROUP_MASK_MASK(trk_gid)) != (uint32)RTL8367C_PORT_TRUNK_GROUP_MASK_MASK(trk_gid))
 ...
 */
rtk_api_ret_t rtk_trunk_distributionAlgorithm_set(rtk_trunk_group_t trk_gid, uint32 algo_bitmask)
{
    rtk_api_ret_t retVal;
 ...
 */
rtk_api_ret_t rtk_trunk_distributionAlgorithm_get(rtk_trunk_group_t trk_gid, uint32 *pAlgo_bitmask)
{
    rtk_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getAsicTrunkingHashSelect((uint32 *)pAlgo_bitmask)) != RT_ERR_OK)
        return retVal;
...
rtk_api_ret_t rtk_trunk_mode_set(rtk_trunk_mode_t mode)
{
    ...
    if ((retVal = rtl8367c_setAsicTrunkingMode((uint32)mode)) != RT_ERR_OK)
        return retVal;
...
rtk_api_ret_t rtk_trunk_mode_get(rtk_trunk_mode_t *pMode)
{
    ...
    if ((retVal = rtl8367c_getAsicTrunkingMode((uint32 *)pMode)) != RT_ERR_OK)
        return retVal;
...
rtk_api_ret_t rtk_trunk_trafficPause_set(rtk_trunk_group_t trk_gid, rtk_enable_t enable)
{
    ...
    if ((retVal = rtl8367c_setAsicTrunkingFc((uint32)trk_gid, (uint32)enable)) != RT_ERR_OK)
        return retVal;
...
rtk_api_ret_t rtk_trunk_trafficPause_get(rtk_trunk_group_t trk_gid, rtk_enable_t *pEnable)
{
    ...
    if ((retVal = rtl8367c_getAsicTrunkingFc((uint32)trk_gid, (uint32 *)pEnable)) != RT_ERR_OK)
        return retVal;
