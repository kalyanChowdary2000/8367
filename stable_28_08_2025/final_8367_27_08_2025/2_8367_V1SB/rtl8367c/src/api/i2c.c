 ...
 *
 * $Revision: 63932 $
 * $Date: 2015-12-08 14:06:29 +0800 (?®‰?, 08 ?Å‰???2015) $
 *
 ...
 */
rtk_api_ret_t rtk_i2c_data_read(uint8 deviceAddr, uint32 slaveRegAddr, uint32 *pRegData)
{
     uint32 retVal, counter=0;
     uint8 controlByte_W, controlByte_R;
     uint8 slaveRegAddr_L, slaveRegAddr_H = 0x0, temp;
     uint8 regData_L, regData_H;

   /* control byte :deviceAddress + W,  deviceAddress + R   */
    controlByte_W = (uint8)(deviceAddr << 1) ;
    controlByte_R = (uint8)(controlByte_W | 0x1);

    slaveRegAddr_L = (uint8) (slaveRegAddr & 0x00FF) ;
    slaveRegAddr_H = (uint8) (slaveRegAddr >>8) ;
 ...
 */
rtk_api_ret_t rtk_i2c_data_write(uint8 deviceAddr, uint32 slaveRegAddr, uint32 regData)
{
     uint32 retVal,counter;
     uint8 controlByte_W;
     uint8 slaveRegAddr_L, slaveRegAddr_H = 0x0, temp;
     uint8 regData_L, regData_H;
  ...
  /* control byte :deviceAddress + W    */
    ...
    regData_H   = (uint8) (regData>> 8);
    regData_L   = (uint8) (regData & 0x00FF);
