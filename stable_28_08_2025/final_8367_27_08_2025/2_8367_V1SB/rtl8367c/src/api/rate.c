...
rtk_api_ret_t rtk_rate_shareMeter_set(rtk_meter_id_t index, rtk_meter_type_t type, rtk_rate_t rate, rtk_enable_t ifg_include)
{
    ...
    /* Set Type */
    if ((retVal = rtl8367c_setAsicShareMeterType(index, (uint32)type)) != RT_ERR_OK)
        return retVal;
...
rtk_api_ret_t rtk_rate_shareMeter_get(rtk_meter_id_t index, rtk_meter_type_t *pType, rtk_rate_t *pRate, rtk_enable_t *pIfg_include)
{
    ...
    if ((retVal = rtl8367c_getAsicShareMeterType(index, (uint32 *)pType)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_rate_shareMeterBucket_set(rtk_meter_id_t index, uint32 bucket_size)
{
    rtk_api_ret_t retVal;
 ...
 */
rtk_api_ret_t rtk_rate_shareMeterBucket_get(rtk_meter_id_t index, uint32 *pBucket_size)
{
    rtk_api_ret_t retVal;
...
rtk_api_ret_t rtk_rate_igrBandwidthCtrlRate_get(rtk_port_t port, rtk_rate_t *pRate, rtk_enable_t *pIfg_include, rtk_enable_t *pFc_enable)
{
    ...
    if ((retVal = rtl8367c_getAsicPortIngressBandwidth(rtk_switch_port_L2P_get(port), &regData, &ifg, &fc)) != RT_ERR_OK)
        return retVal;
    *pRate = regData<<3;
    *pIfg_include = ifg;
    *pFc_enable = fc;
...
rtk_api_ret_t rtk_rate_egrBandwidthCtrlRate_get(rtk_port_t port, rtk_rate_t *pRate, rtk_enable_t *pIfg_include)
{
    ...
    *pRate = regData << 3;
    ...
    if ((retVal = rtl8367c_getAsicPortEgressRateIfg(&ifg)) != RT_ERR_OK)
        return retVal;
    *pIfg_include = ifg;
