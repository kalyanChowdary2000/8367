 ...
 */
rtk_api_ret_t rtk_l2_addr_next_get(rtk_l2_read_method_t read_method, rtk_port_t port, uint32 *pAddress, rtk_l2_ucastAddr_t *pL2_data)
{
    rtk_api_ret_t   retVal;
    uint32      method;
    rtl8367c_luttb  l2Table;
 ...
 */
rtk_api_ret_t rtk_l2_mcastAddr_next_get(uint32 *pAddress, rtk_l2_mcastAddr_t *pMcastAddr)
{
    rtk_api_ret_t   retVal;
    rtl8367c_luttb  l2Table;
 ...
 */
rtk_api_ret_t rtk_l2_ipMcastAddr_next_get(uint32 *pAddress, rtk_l2_ipMcastAddr_t *pIpMcastAddr)
{
    rtk_api_ret_t   retVal;
    rtl8367c_luttb  l2Table;
 ...
 *      as pAddress to call this API again for dumping all IP multicast entries is LUT.
 */
rtk_api_ret_t rtk_l2_ipVidMcastAddr_next_get(uint32 *pAddress, rtk_l2_ipVidMcastAddr_t *pIpVidMcastAddr)
{
    rtk_api_ret_t   retVal;
    rtl8367c_luttb  l2Table;
...
rtk_api_ret_t rtk_l2_table_clearStatus_get(rtk_l2_clearStatus_t *pStatus)
{
    ...
    if ((retVal = rtl8367c_getAsicLutFlushAllStatus((uint32 *)pStatus)) != RT_ERR_OK)
        return retVal;
...
rtk_api_ret_t rtk_l2_limitLearningCntAction_set(rtk_port_t port, rtk_l2_limitLearnCntAction_t action)
{
    ...
    if ((retVal = rtl8367c_setAsicLutLearnOverAct(data1)) != RT_ERR_OK)
        return retVal;
...
rtk_api_ret_t rtk_l2_limitSystemLearningCntAction_set(rtk_l2_limitLearnCntAction_t action)
{
    ...
    if ((retVal = rtl8367c_setAsicSystemLutLearnOverAct(data1)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_l2_ipMcastGroupEntry_add(ipaddr_t ip_addr, uint32 vid, rtk_portmask_t *pPortmask)
{
    uint32      empty_idx = 0xFFFF;
    uint32       index;
    ipaddr_t        group_addr;
    uint32      group_vid;
    uint32      pmask;
    uint32      valid;
    uint32      physicalPortmask;
    rtk_api_ret_t   retVal;
    ...
    {
        if ((retVal = rtl8367c_getAsicLutIPMCGroup((uint32)index, &group_addr, &group_vid, &pmask, &valid))!=RT_ERR_OK)
            return retVal;
 ...
 *      Delete an entry from IP Multicast group table.
 */
rtk_api_ret_t rtk_l2_ipMcastGroupEntry_del(ipaddr_t ip_addr, uint32 vid)
{
    uint32       index;
    ipaddr_t        group_addr;
    uint32      group_vid;
 ...
 *      Delete an entry from IP Multicast group table.
 */
rtk_api_ret_t rtk_l2_ipMcastGroupEntry_get(ipaddr_t ip_addr, uint32 vid, rtk_portmask_t *pPortmask)
{
    uint32       index;
    ipaddr_t        group_addr;
    uint32      group_vid;
