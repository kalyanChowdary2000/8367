 ...
 *
 * $Revision: 68033 $
 * $Date: 2016-05-12 14:48:03 +0800 (?Ÿæ??? 12 äº”æ? 2016) $
 *
...
typedef struct rtk_switch_halCtrl_s
{
    ...
    uint32      l2p_port[RTK_SWITCH_PORT_NUM];
    uint32      p2l_port[RTK_SWITCH_PORT_NUM];
    port_type_t     log_port_type[RTK_SWITCH_PORT_NUM];
    uint32      ptp_port[RTK_SWITCH_PORT_NUM];
    uint32      valid_portmask;
    uint32      valid_utp_portmask;
    uint32      valid_ext_portmask;
    uint32      valid_cpu_portmask;
    uint32      min_phy_port;
    uint32      max_phy_port;
    uint32      phy_portmask;
    uint32      combo_logical_port;
    uint32      hsg_logical_port;
    uint32      sg_logical_portmask;
    uint32      max_meter_id;
    uint32      max_lut_addr_num;
    uint32      trunk_group_mask;
...
#define RTK_PORTMASK_IS_PORT_SET(__portmask__, __port__)    (((__portmask__).bits[0] & (1UL/*0x0001*/ << __port__)) ? 1 : 0)
...
#define RTK_PORTMASK_PORT_SET(__portmask__, __port__)       ((__portmask__).bits[0] |= (1UL/*0x0001*/ << __port__))
#define RTK_PORTMASK_PORT_CLEAR(__portmask__, __port__)     ((__portmask__).bits[0] &= ~(1UL/*0x00000001*/ << __port__))
...
#define RTK_SCAN_ALL_PHY_PORTMASK(__port__)                 for(__port__ = 0; __port__ < RTK_SWITCH_PORT_NUM; __port__++)  if( (rtk_switch_phyPortMask_get() & (1UL/*0x0001*/ << __port__)))
 ...
 *
 */
extern uint32 rtk_switch_ComboPort_get(void);
 ...
 *
 */
extern uint32 rtk_switch_port_L2P_get(rtk_port_t logicalPort);
 ...
 *
 */
extern rtk_port_t rtk_switch_port_P2L_get(uint32 physicalPort);
 ...
 *
 */
extern rtk_api_ret_t rtk_switch_portmask_L2P_get(rtk_portmask_t *pLogicalPmask, uint32 *pPhysicalPortmask);
 ...
 *
 */
extern rtk_api_ret_t rtk_switch_portmask_P2L_get(uint32 physicalPortmask, rtk_portmask_t *pLogicalPmask);
 ...
 * Note:
 */
extern rtk_api_ret_t rtk_switch_portMaxPktLen_set(rtk_port_t port, rtk_switch_maxPktLen_linkSpeed_t speed, uint32 cfgId);
 ...
 * Note:
 */
extern rtk_api_ret_t rtk_switch_portMaxPktLen_get(rtk_port_t port, rtk_switch_maxPktLen_linkSpeed_t speed, uint32 *pCfgId);
 ...
 * Note:
 */
extern rtk_api_ret_t rtk_switch_maxPktLenCfg_set(uint32 cfgId, uint32 pktLen);
 ...
 * Note:
 */
extern rtk_api_ret_t rtk_switch_maxPktLenCfg_get(uint32 cfgId, uint32 *pPktLen);
 ...
 *
 */
extern uint32 rtk_switch_maxMeterId_get(void);
 ...
 *
 */
extern uint32 rtk_switch_maxLutAddrNumber_get(void);
