...
typedef struct
{
    ...
    /* CVLAN acton */
    uint32      filterCvlanVid;
    uint32      filterCvlanIdx;
    /* SVLAN action */
    uint32      filterSvlanVid;
    uint32      filterSvlanIdx;
...
struct rtk_filter_field
{
    ...
    uint32 fieldTemplateNo;
    uint32 fieldTemplateIdx[RTK_FILTER_FIELD_USED_MAX];
 ...
 */
extern rtk_api_ret_t rtk_filter_igrAcl_field_sel_set(uint32 index, rtk_field_sel_t format, uint32 offset);
 ...
 *      None.
 */
extern rtk_api_ret_t rtk_filter_igrAcl_field_sel_get(uint32 index, rtk_field_sel_t *pFormat, uint32 *pOffset);
 ...
 */
extern rtk_api_ret_t rtk_filter_iprange_set(uint32 index, rtk_filter_iprange_t type, ipaddr_t upperIp, ipaddr_t lowerIp);
 ...
 *      upperIp must be larger or equal than lowerIp.
 */
extern rtk_api_ret_t rtk_filter_iprange_get(uint32 index, rtk_filter_iprange_t *pType, ipaddr_t *pUpperIp, ipaddr_t *pLowerIp);
 ...
 */
extern rtk_api_ret_t rtk_filter_vidrange_set(uint32 index, rtk_filter_vidrange_t type, uint32 upperVid, uint32 lowerVid);
 ...
 *      None.
 */
extern rtk_api_ret_t rtk_filter_vidrange_get(uint32 index, rtk_filter_vidrange_t *pType, uint32 *pUpperVid, uint32 *pLowerVid);
 ...
 */
extern rtk_api_ret_t rtk_filter_portrange_set(uint32 index, rtk_filter_portrange_t type, uint32 upperPort, uint32 lowerPort);
 ...
 *      None.
 */
extern rtk_api_ret_t rtk_filter_portrange_get(uint32 index, rtk_filter_portrange_t *pType, uint32 *pUpperPort, uint32 *pLowerPort);
 ...
 *      none
 */
extern rtk_api_ret_t rtk_filter_igrAclPolarity_set(uint32 polarity);
 ...
 *      none
 */
extern rtk_api_ret_t rtk_filter_igrAclPolarity_get(uint32* pPolarity);
