 ...
 *      - rtk_svlan_memberCfg_t->priority is priority of SVLAN member configuration.
 */
extern rtk_api_ret_t rtk_svlan_memberPortEntry_adv_set(uint32 idx, rtk_svlan_memberCfg_t *pSvlan_cfg);
 ...
 *      to receiving from uplink ports. Other SVID S-tag frame or S-untagged frame will be droped.
 */
extern rtk_api_ret_t rtk_svlan_memberPortEntry_adv_get(uint32 idx, rtk_svlan_memberCfg_t *pSvlan_cfg);
 ...
 *
 */
extern rtk_api_ret_t rtk_svlan_checkAndCreateMbr(rtk_vlan_t vid, uint32 *pIndex);
